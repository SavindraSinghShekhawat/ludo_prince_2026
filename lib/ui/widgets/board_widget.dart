      ...
      builder: (context, constraints) {
        ...
        return RepaintBoundary(
          child: SizedBox(
            width: boardSize,
            height: boardSize,
            child: CustomPaint(painter: StaticBoardPainter(gameMode: gameMode)),
          ),
        );
      },
    );
...
class StaticBoardPainter extends CustomPainter {
  ...
  void _drawBaseArea(
    ...
    void drawSpot(double dx, double dy) {
      final spotCenter = Offset(
        rect.left + dx * cellSize,
        rect.top + dy * cellSize,
      );
  ...
  void _drawCenterHome(Canvas canvas, Size size, double cellSize) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(startX, startY, w, h),
  ...
  Color _getCellColor(int col, int row) {
    if (row == 7 && col >= 1 && col <= 5) {
      return AppColors.player4Red.withValues(
        alpha: AppColors.boardHomeStretchAlpha,
      );
    }
    if (col == 7 && row >= 1 && row <= 5) {
      return AppColors.player3Green.withValues(
        alpha: AppColors.boardHomeStretchAlpha,
      );
    }
    if (row == 7 && col >= 9 && col <= 13) {
      return AppColors.player2Yellow.withValues(
        alpha: AppColors.boardHomeStretchAlpha,
      );
    }
    if (col == 7 && row >= 9 && row <= 13) {
      return AppColors.player1Blue.withValues(
        alpha: AppColors.boardHomeStretchAlpha,
      );
    }
    if (col == 1 && row == 6) {
      return AppColors.player4Red.withValues(
        alpha: AppColors.boardStartCellAlpha,
      );
    }
    if (col == 8 && row == 1) {
      return AppColors.player3Green.withValues(
